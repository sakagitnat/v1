"""Grid-search RsiReversionStrategy's parameters against real
forex/gold history, using the same TRAIN/TEST/overfit-safety discipline
as optimize_cfd_mean_reversion.py.

RSI reversion is structurally distinct from Bollinger-band mean
reversion: it fades momentum exhaustion rather than price-band
deviation, with a strict ADX gate to keep entries confined to genuinely
flat markets. This script treats every default as unvalidated until the
same holdout discipline used elsewhere in the project says otherwise.

Needs a live network connection -- run via GitHub Actions and inspect the
job logs.

Usage: python scripts/optimize_cfd_rsi_reversion.py [--granularity 3600] [--source deriv|yfinance]
"""
import argparse
import asyncio
import itertools
...
def run_backtest(strategy_kwargs: dict, bars: dict) -> dict:
    strategy = RsiReversionStrategy(**strategy_kwargs)
...
async def main(granularity_seconds: int, source: str):
    ...
    print("\nBaseline (RsiReversionStrategy default params, unvalidated placeholders):")
    ...
    if not pool:
        print(f"\nNo combination qualified (min {MIN_TRADES} trades, CAGR>0, drawdown cap). RsiReversionStrategy does not look promising on this data.")
        return
    ...
    if not robust:
        print(
            ...
            "RsiReversionStrategy does not look promising on this data either."
        )
        return
