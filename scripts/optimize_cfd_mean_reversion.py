"""Grid-search MeanReversionStrategy's parameters against real
...
Why this strategy: every registered strategy so far (EmaCrossoverStrategy,
DonchianBreakoutStrategy) is trend-following, so trading.cfd.regime's
RANGING classification has had no registered strategy suited to it --
every ranging period was a NO TRADE by omission, not design (see
docs/ARCHITECTURE_AUDIT.md gap #11). trading.cfd.mean_reversion's
MeanReversionStrategy fades price extremes back toward a Bollinger Band
mean instead of following a breakout/crossover -- a genuinely different
structural bet, real diversification against the existing pool rather
than another correlated trend-following variant.

Needs a live network connection -- run via the "CFD Manual Command"
GitHub Actions workflow and read its job logs.

Usage: python scripts/optimize_cfd_mean_reversion.py [--granularity 3600] [--source deriv|yfinance]
"""
import argparse
import asyncio
import itertools
...
PARAM_GRID = {
    # 20/2.0 is the textbook Bollinger Band default (John Bollinger's
    # own), widened/narrowed around it; atr_stop_mult/atr_target_mult
    # mirror the shape of the other two strategies' grids so this sweep
    # is comparable, not a differently-shaped search.
    "band_window": [10, 20, 30],
    "band_std": [1.5, 2.0, 2.5],
    "atr_stop_mult": [1.5, 2.0, 3.0],
    "atr_target_mult": [1.0, 1.5, 2.5],
}


def run_backtest(strategy_kwargs: dict, bars: dict) -> dict:
    strategy = MeanReversionStrategy(**strategy_kwargs)
...
async def main(granularity_seconds: int, source: str):
    ...
    print("\nBaseline (MeanReversionStrategy default params, unvalidated placeholders):")
    ...
    if not pool:
        print(f"\nNo combination qualified (min {MIN_TRADES} trades, CAGR>0, drawdown cap). MeanReversionStrategy does not look promising on this data.")
        return
    ...
    if not robust:
        print(
            ...
            "MeanReversionStrategy does not look promising on this data either."
        )
        return
