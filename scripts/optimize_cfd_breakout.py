"""Grid-search DonchianBreakoutStrategy's parameters against real
forex/gold history, same TRAIN/TEST/overfit-safety discipline as
optimize_cfd_strategy.py (see that file's docstring for the full
methodology -- reused here via import, not copy-pasted).

Why this strategy, not another EmaCrossoverStrategy parameter sweep:
every risk level tried on EmaCrossoverStrategy across forex/gold,
crypto, and Deriv's synthetic indices (see README's "Backtesting"
section) showed the same pattern -- TRAIN and TEST flipping between
catastrophic and spectacular depending on which period caught the
trend, never both survivable at once. That's a thin, inconsistent
edge no amount of position sizing fixes; the lever that's actually
untried is the entry logic itself. BreakoutStrategy (Donchian channel,
"Turtle Trading") won decisively in the stock side of this project's
own strategy comparison (beat a regime-switching ensemble on CAGR,
Sharpe, AND max drawdown at once), so trading.cfd.breakout's
DonchianBreakoutStrategy adapts it here (long+short, since Multipliers
support both) as a genuinely different structural bet, not a retune.

Needs a live network connection -- run via the "CFD Manual Command"
GitHub Actions workflow and read its job logs.

Usage: python scripts/optimize_cfd_breakout.py [--granularity 3600] [--source deriv|yfinance]
"""
import argparse
import asyncio
import itertools
...
PARAM_GRID = {
    ...
    # starting anchor, not guessed from nothing -- widened/narrowed
    # around them since this is H1 (hourly) bars, not the daily bars
    # the original system and BreakoutStrategy's own stock-side
    # defaults were built for.
    "entry_window": [20, 30, 55, 80],
    "exit_window": [10, 15, 20],
    "atr_stop_mult": [1.5, 2.5, 3.5],
    "atr_target_mult": [3.0, 4.5, 6.0],
}


def run_backtest(strategy_kwargs: dict, bars: dict) -> dict:
    strategy = DonchianBreakoutStrategy(**strategy_kwargs)
...
async def main(granularity_seconds: int, source: str):
    ...
    print("\nBaseline (DonchianBreakoutStrategy default params, unvalidated placeholders):")
    ...
    ]  # entry_window > exit_window, or entries would be easier to trigger than exits
    ...
    if not pool:
        print(f"\nNo combination qualified (min {MIN_TRADES} trades, CAGR>0, drawdown cap). DonchianBreakoutStrategy does not look promising on this data.")
        return
    ...
    if not robust:
        print(
            ...
            "DonchianBreakoutStrategy does not look promising on this data either."
        )
        return
